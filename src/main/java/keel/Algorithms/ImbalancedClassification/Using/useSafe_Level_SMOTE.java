...
import keel.Algorithms.ImbalancedClassification.Resampling.Safe_Level_SMOTE;

public class useSafe_Level_SMOTE {

    public void mainSafe_Level_SMOTE(){
        String configFile;
        utils scan1 = new utils();
        System.out.println("Safe_Level_SMOTE Configuration File");
        ...
        configFile = "Safe_Level_SMOTE_" + trainFile + ".txt";
        CreateImblearnConfigFile createFile = new CreateImblearnConfigFile();
        createFile.createConfigFile4("Safe_Level_SMOTE", trainFile, testFile,
                trainResultFile, testResultFile, seed, neighbors, SMOTEtype,
                balance, quantity, distance, interpolation, alpha, mu);
        Safe_Level_SMOTE safe_level_smote = new Safe_Level_SMOTE(configFile);
        safe_level_smote.run();
    }
